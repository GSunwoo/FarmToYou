...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.review-cards').forEach(card => { //카드가 여러개라 모든 카드에 클릭 이벤트 생성
	card.addEventListener('click', () => { //card는 각각의 DOM 요소
		...
		const star = +card.dataset.star || 0; // 카드에 박아둔 data-star 값을 숫자로 변환(+는 빠른 number캐스팅)
		...
		modalDate.setAttribute('datetime', dateIso); //datetime 속성을 ISO 형식 날짜로 설정
		modalDate.textContent = dateHuman || ''; //사람용 설정
		...
		for(let i=1; i<=5; i++){
			modalRating.innerHTML += i <= star
				? '<i class="fa-solid fa-star"></i>'
				: '<i class="fa-regular fa-star"></i>'
			//i <= star면 채운 별,, 아니면 빈 별 아이콘 추가
		}
  ...
  modal.querySelector('.modal.close').addEventListener('click', () => { //닫기 버튼 클릭시
	modal.setAttribute('hidden', ''); // hidden을 다시 줌
  });
  ...
  likeBtn.addEventListener('click', () => {
	...
	//이 리뷰의 좋아요 수를 1증가 시켜달라는 의미.
		.then(res => res.json())
		.then(data => { 
			if (typeof data.review_like !== 'undefined') likeCount.textContent = data.review_like;
		})//화면에 바로 새로운 좋아요 수 반영
